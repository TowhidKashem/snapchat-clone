...
const JeelizThreeGlassesCreator = function (spec) {
  ...
  // glasses frames:
  new THREE.BufferGeometryLoader().load(spec.frameMeshURL, function (
    glassesFramesGeometry
  ) {
    ...
    let fragmentShaderSource =
      'uniform vec2 uBranchFading;\n varying float vPosZ;\n' +
      THREE.ShaderLib.standard.fragmentShader;
    const GLSLcomputeAlpha =
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      transparent: true,
      extensions: {
        // fix for https://github.com/jeeliz/jeelizFaceFilter/issues/154
        //derivatives: true,
  ...
  // glasses lenses:
  new THREE.BufferGeometryLoader().load(spec.lensesMeshURL, function (
    glassesLensesGeometry
  ) {
