...
const SETTINGS = {
  ...
  detectedStates: [
    { x: -0.359, y: 0.322, s: 0.175, ry: -0.033, rz: 0.239 },
    { x: -0.34, y: 0.32, s: 0.171, ry: -0.024, rz: 0.239 },
    { x: -0.277, y: 0.31, s: 0.173, ry: -0.059, rz: 0.221 },
    { x: -0.256, y: 0.316, s: 0.174, ry: -0.077, rz: 0.203 },
    { x: -0.242, y: 0.321, s: 0.173, ry: -0.107, rz: 0.187 },
    { x: -0.24, y: 0.334, s: 0.168, ry: -0.074, rz: 0.165 },
    { x: -0.239, y: 0.347, s: 0.164, ry: -0.061, rz: 0.148 },
    { x: -0.235, y: 0.352, s: 0.162, ry: -0.058, rz: 0.133 },
    { x: -0.233, y: 0.354, s: 0.161, ry: -0.06, rz: 0.133 },
    { x: -0.228, y: 0.356, s: 0.163, ry: -0.067, rz: 0.138 },
    { x: -0.234, y: 0.352, s: 0.166, ry: -0.089, rz: 0.154 },
    { x: -0.233, y: 0.364, s: 0.168, ry: -0.076, rz: 0.148 },
    { x: -0.216, y: 0.37, s: 0.174, ry: -0.098, rz: 0.145 },
    { x: -0.191, y: 0.373, s: 0.175, ry: -0.132, rz: 0.139 },
    { x: -0.131, y: 0.388, s: 0.178, ry: -0.134, rz: 0.102 },
    { x: -0.091, y: 0.384, s: 0.178, ry: -0.146, rz: 0.096 },
    { x: -0.062, y: 0.383, s: 0.177, ry: -0.157, rz: 0.079 },
    { x: -0.01, y: 0.363, s: 0.179, ry: -0.109, rz: 0.08 },
    { x: 0.049, y: 0.345, s: 0.178, ry: -0.174, rz: 0.032 },
    { x: 0.124, y: 0.336, s: 0.182, ry: -0.25, rz: -0.052 }
  ],
  mirroredLoop: true, //*/
  ...
  gifMaskOffset: [0.01, 0.1], // relative. 1-> 100% scale mask width of the image (or height)
...
function update_gif(detectedStates) {
  ...
  if (detectedStates) {
    ...
    while (detectedStates.length < GIF.frames.length) {
      // complete lacking frames with false
...
function size_CSSgif() {
  ...
  let w = 'auto',
    h = 'auto';
  if (aspectRatioGif > aspectRatioAvailableSpace) {
    // the gif is in landscape
    w = Math.round(0.9 * window.innerWidth).toString() + 'px';
  } else {
    // the gif is in portrait
...
function change_gif(
  ...
  detectedStates,
  isMirroredLoop,
  hideIfNotDetectedDuringNframes
) {
  if (
    urlImage === GIF.url ||
    [STATES.DETECTGIFFACE, STATES.DETECTUSERFACE].indexOf(STATE) === -1
  ) {
    return;
  }
  ...
  if (urlImage === 'CUSTOM') {
    ...
    // upload image from the carousel:
    GIF.mirroredLoop = isMirroredLoop ? true : false;
    GIF.hideIfNotDetectedDuringNframes = hideIfNotDetectedDuringNframes
      ? hideIfNotDetectedDuringNframes
      : false;
...
function load_gifBlob(blob, callback) {
  // called both at first loading and by change_gif()
  ...
  GIF.image.onload = function () {
    ...
    fileReader.onload = function (event) {
      ...
      GIF.info.duration =
        GIF.info.duration ||
        GIF.info.durationChrome ||
        GIF.info.durationFirefox ||
        GIF.info.durationIE ||
        GIF.frames.length * 20;
...
function interpolate_detectedStates() {
  if (
    GIF.detectedStates.every(function (ds) {
      return !ds;
    })
  ) {
  ...
  const mix_states = function (sa, sb, t) {
    // analogous to GLSL mix function
    return {
      x: sa.x * (1 - t) + sb.x * t,
      y: sa.y * (1 - t) + sb.y * t,
      s: sa.s * (1 - t) + sb.s * t,
  ...
  const get_dState = function (ind) {
    return GIF.detectedStates[
      (ind + GIF.detectedStates.length) % GIF.detectedStates.length
    ];
  };
  ...
  GIF.detectedStates = GIF.detectedStates.map(function (ds, dsi) {
    ...
    if (
      GIF.hideIfNotDetectedDuringNframes &&
      nNotDetectedFrames > GIF.hideIfNotDetectedDuringNframes
    ) {
      return false;
    }
...
function build_gifMasks(detectStates) {
  // detectStates is the detectState for each gif frame
...
function build_gifFrameMask(detectState, frameIndex) {
  ...
  const x = detectState.x,
    ...
    s = detectState.s,
    ry = detectState.ry,
    rz = detectState.rz ? detectState.rz : 0;
  ...
  const shpBuildMask = build_shaderProgram(
    ...
    'precision highp float;\n\
     ...
     const float UPPERHEADY=' +
      SETTINGS.gifHeadForheadY.toFixed(2) +
      ';\n\
     const float LOWERHEADY=' +
      SETTINGS.gifHeadJawY.toFixed(2) +
      ';\n\
     const float SMOOTHEDGE=' +
      SETTINGS.gifCropSmoothEdge.toFixed(2) +
      ';\n\
     \n\
     \n\
     void main(void){\n\
...
function update_positionUserCropCanvas(frameIndex) {
  // called when the GIF frame changes when played
...
function draw_render(detectState) {
  //detectState is the detectState of the USER (not the GIF)
  // do RTT:
  GL.bindFramebuffer(GLDRAWTARGET, FBO);
  ...
  const xn =
    detectState.x * 0.5 + 0.5 + s * SETTINGS.gifMaskOffset[0] * Math.sin(detectState.ry); //normalized x position
...
function callbackTrack(detectState) {
  switch (STATE) {
    case STATES.DETECTGIFFACE: // search for a face
      ...
      ++GIF.testCounter;
      if (GIF.testCounter > SETTINGS.nMaxTestsGif) {
        // no face has been detected, go to the next frame (face position will be interpolated later)
      ...
      if (detectState.detected > SETTINGS.detectGifThreshold) {
      ...
      if (GIF.detectedStates.length === GIF.frames.length) {
        // the face has been detected on all frame of the gif
    ...
    case STATES.DETECTUSERFACE:
      ...
      let currentFrameIndex = Math.floor(
        (GIF.frames.length * (currentTimestamp - GIF.animation.startTimestamp)) /
          GIF.info.duration
      );
