...
function init_scene(spec) {
  ...
  const perspectivePx = Math.round(
    Math.pow(w2 * w2 + h2 * h2, 0.5) / Math.tan((SETTINGS.cameraFOV * Math.PI) / 180)
  );
  ...
  MOVEMENT = {
    ...
    pivotOffset0: new THREE.Vector3(
      0,
...
function main() {
  ...
  if (!DIV) {
    alert(
      'ERROR : You should have an element which id=jeelizFaceFilterFollow in the DOM. Abort.'
    );
    return;
  }
  ...
  JEEFACEFILTERAPI.init({
    ...
    callbackTrack: function (detectState) {
      ...
      if (ISDETECTED) {
        ...
        if (
          ISMOUTHOPENED &&
          mouthOpening < SETTINGS.mouthOpeningThreshold - SETTINGS.mouthOpeningHysteresis
        ) {
          // user closes mouth
